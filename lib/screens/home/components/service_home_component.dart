...
class ServicesHomeComponent extends StatelessWidget {
  ServicesHomeComponent({Key? key}) : super(key: key);
  ...
  Future<void> handleStatusChangeClick({required BuildContext context, required bool statusValue, required int index}) async {
    ...
    showConfirmDialogCustom(
      ...
      onAccept: (ctx) async {
        ...
        ServiceFormApis.updateServicesStatus(serviceId: homeController.dashboardData.value.data.doctorServices[index].id, request: {"status": homeController.dashboardData.value.data.doctorServices[index].status.value.getIntBool()}).then((value) {
