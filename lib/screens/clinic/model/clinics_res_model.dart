class ClinicListRes {
  ...
  factory ClinicListRes.fromJson(Map<String, dynamic> json) {
    ...
    print(
        "ClinicListRes.fromJson received structure with keys: ${json.keys.toList()}");

    // Handle the new structure with 'success' field and nested data.data array
      ...
      if (json['data'] is Map && json['data'].containsKey('data')) {
        ...
        if (nestedData is List) {
          ...
          return ClinicListRes(
            status: json['success'] is bool ? json['success'] : false,
            data: List<ClinicData>.from(
                nestedData.map((x) => ClinicData.fromJson(x))),
    ...
    if (json.containsKey('clinics') && json['clinics'] is List) {
      return ClinicListRes(
        status: true,
        data: List<ClinicData>.from(
            json['clinics'].map((x) => ClinicData.fromJson(x))),
    ...
    map.forEach((key, value) {
      if (value is Map<String, dynamic>) {
        ...
        if (value.containsKey('id') &&
            (value.containsKey('name') || value.containsKey('label'))) {
          ...
          try {
            ...
            print(
                "Extracted clinic with ID: ${value['id']} and name: ${value['name']}");
          } catch (e) {
...
class ClinicData {
  ...
  factory ClinicData.fromJson(Map<String, dynamic> json) {
    ...
    if (json['status'] is int) {
      ...
      statusValue =
          json['status']?.toString().toLowerCase() == 'active' ? 1 : 0;
    }
    ...
    return ClinicData(
      ...
      systemServiceCategory: json['system_service_category'] is int
      ...
      distanceFormate:
          json['distance_formate'] is String ? json['distance_formate'] : "",
      status: statusValue,
      clinicStatus: json['status'] is String ? json['status'] : "",
      clinicImage: json['clinic_image'] is String
          ? json['clinic_image']
