...
class AuthServiceApis {
  ...
  static Future<UserResponse> loginUser({required Map request, bool isSocialLogin = false}) async {
    var response = await handleResponse(await buildHttpResponse(isSocialLogin ? APIEndPoints.socialLogin : APIEndPoints.login, request: request, method: HttpMethodType.POST));
    ...
    if (response['status'] is int || (response['data'] is Map && response['data']['user'] is Map)) {
      // Extract the token from the response
      String token = "";
      if (response['data'] is Map && response['data']['token'] != null) {
        token = response['data']['token'].toString();
      } else if (response['data'] is Map && response['data']['user'] is Map && response['data']['user']['token'] != null) {
      ...
      var transformedResponse = {
        'status': response['status'] == 200 || response['status'] == true, // Convert HTTP status to boolean
        'message': response['message'] ?? 'Login successful',
        'data': {
          // Create 'data' with expected properties for UserData.fromJson
          'id': userData['id'] ?? -1,
          'first_name': userData['name']?.toString().split(' ').first ?? userData['first_name'] ?? '',
          ...
          'profile_image': userData['avatar'] ?? userData['profile_image'] ?? '',
          'api_token': token,
      ...
      userResponse.data = response['data'] is Map ? Map<String, dynamic>.from(response['data']) : null;
  ...
  static Future<void> clearData({bool isFromDeleteAcc = false}) async {
   ...
   // PushNotificationService().unsubscribeFirebaseTopic();
    if (isFromDeleteAcc) {
      ...
      final tempIsRemeberMe = getValueFromLocal(SharedPreferenceConst.REMEMBER_USER);
  ...
  static Future<dynamic> updateProfile({
    ...
    if (isLoggedIn.value) {
      ...
      if(dateOfBirth.isNotEmpty) multiPartRequest.fields[UserKeys.dateOfBirth] = dateOfBirth;
      if (country.isNotEmpty) multiPartRequest.fields[UserKeys.country] = country;
      ...
      if (pinCode.isNotEmpty) multiPartRequest.fields[UserKeys.pinCode] = pinCode;
